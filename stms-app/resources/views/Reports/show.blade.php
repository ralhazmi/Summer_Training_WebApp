...
<a href="{{route('Reportsindex')}}" class="inline-flex items-end text-lg text-blue-900 hover:underline">
...
<div class="flex justify-center">
  <div class="max-w-sm pb-60 bg-white border border-gray-200 rounded-lg shadow"style="width: 400px;">
    ...
    <div class="text-gray-400 font-semibold flex justify-end mb-5 "style="margin-bottom: -25px;">{{$report->date}}</div>
      <h1 class="text-2xl font-serif mb-5"style="color:#00519B;"> {{$report->report_title}}</h1>
      <div class="font-normal">
      @if(auth()->user()->role == 1)
        <div class="font-semibold"style="color:#00519B;">To: {{ $userToName->username }}</div>
    @elseif(auth()->user()->role == 2)
        <div class="font-semibold"style="color:#00519B;">From: {{ $report->user->username }}</div>
    @endif
    ...
    <button onclick="window.location='{{ route('Reportsdownload', $report->attachment) }}'" class="bg-blue-900 hover:bg-blue-700 text-white font-bold text-sm py-2 px-4 rounded flex items-center">
        Download File
